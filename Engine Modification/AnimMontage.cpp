			...
			while (bPlaying && MontageSubStepper.HasTimeRemaining() && (++NumIterations < MaxIterations))
			{
				...
				if (!IsStopped() && bEnableAutoBlendOut)
				{
					...
					if (NextSectionIndex == INDEX_NONE)
					{
						...
						if (PlayTimeToEnd <= FMath::Max<float>(BlendOutTriggerTime, KINDA_SMALL_NUMBER))
						{
							...
							//Charlie - Custom Animation Support - Blend Out
							if (bCustomAnimationBlendOut)
							{
								Stop(FAlphaBlend(Montage->BlendOut, BlendOutTime), false);
							}
							else
							{
								Stop(FAlphaBlend(Montage->BlendOut, 0.0f), false);
							}
							//~Charlie
						}
					}
				...
				if(MontageSubStepper.HasTimeRemaining())
				{
					...
					if (MontageSubStepper.HasReachedEndOfSection() && !BranchingPointMarker && (PositionBeforeFiringEvents == Position))
					{
						...
						int32 RecentNextSectionIndex = bPlayingForward ? NextSections[CurrentSectionIndex] : PrevSections[CurrentSectionIndex];
						const int32 LoopingSectionIndex = Montage->GetSectionIndex(FName(TEXT("Loop")));
						if (CurrentSectionIndex == LoopingSectionIndex)
						{
							//Note: We check >1 here, as to avoid confusing an input of "1 loop" should result int he animation playing from start to end a single time.
							//This means that both "0 loops" and "1 loop" will result in the same nnumber of playbacks
							if (CustomAnimationLoopingSectionLoops == -1)
							{
								RecentNextSectionIndex = CurrentSectionIndex;
							}
							if (CustomAnimationLoopingSectionLoops > 1)
							{
								RecentNextSectionIndex = CurrentSectionIndex;
								CustomAnimationLoopingSectionLoops--;
							}
						}
						//~Charlie
						if (RecentNextSectionIndex != INDEX_NONE)
						{
							...
							if (Inst)
							{
								Inst->QueueMontageSectionEndedEvent(FQueuedMontageSectionEndedEvent(Montage, CurrentSectionIndex, RecentNextSectionIndex, OnMontageSectionEnded, InstanceID));
							}
							//~Charlie
						}
						else
...
UAnimMontage* UAnimMontage::CreateSlotAnimationAsDynamicMontage(UAnimSequenceBase* Asset, FName SlotNodeName, float BlendInTime, float BlendOutTime, float InPlayRate, int32 LoopCount, float BlendOutTriggerTime, float InTimeToStartMontageAt)
{
	...
	//Charlie - Custom Animation Support - Custom Loops
	//Note: Looping count should not ever be a value that is less than 1, as it is used in calculating Montage length
	//Also, other parts of pre-existing UE4 code expect this value to be at least 1 (though they do carry out checks)

	//If this logic needs to be changed, note that is is also carried out in "UAnimInstance::PlaySlotAnimation" and will need to be changed there too.
